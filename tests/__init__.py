...
def sample_df(spark):
    ...
    data = [
        (1,  2, " Graduate ",   " No ",  800000,  500000,  12, 720, 1000000, 500000, 200000, 300000, " Approved "),
        (2,  0, " Not Graduate"," Yes ", 300000, 1500000,  24, 480, 200000,  100000,  50000, 100000, " Rejected "),
        (3,  1, " Graduate ",   " No ",  600000,  800000,  18, 680, 800000,  300000, 150000, 250000, " Approved "),
        (4,  3, " Not Graduate"," Yes ", 150000, 2000000,  36, 390, 100000,   50000,  20000,  80000, " Rejected "),
        (5,  1, " Graduate ",   " No ", 1200000,  700000,   6, 780, 2000000, 1000000, 500000, 600000, " Approved "),
    ]
    columns = [
...
def test_string_trimming(sample_df):
    ...
    assert all(" " not in e for e in educations),  "education has leading/trailing spaces"
    assert all(" " not in s for s in statuses),    "loan_status has leading/trailing spaces"
    assert all(e == e.lower() for e in educations), "education not lowercased"
...
def test_is_approved_flag(sample_df):
    ...
    assert set(flags).issubset({0, 1}), "is_approved contains values other than 0 and 1"
    assert sum(flags) == 3, "Expected 3 approved loans in sample"
...
def test_debt_to_income_positive(sample_df):
    ...
    assert all(r > 0 for r in ratios), "debt_to_income should always be positive"
...
def test_total_assets_calculation(sample_df):
    ...
    assert first == 2000000.0, f"Expected 2000000.0, got {first}"
...
def test_cibil_band_assignment(sample_df):
    ...
    bands = {r.cibil_score: r.cibil_band
    ...
    assert bands[680.0] == "Good (650-749)"
...
def test_income_tier_assignment(sample_df):
    ...
    tiers = {r.income_annum: r.income_tier
...
def test_agg_approval_by_education_columns(sample_df):
    ...
    expected_cols = {"education", "loan_status", "count",
